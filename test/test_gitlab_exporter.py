...
from gitlab_downloader import GitlabExporter
...
class TestExportProject(unittest.TestCase):
    ...
    def test_export_project_success(self, mock_open, mock_get, mock_post):
        # Mock the initiation of the export
        mock_post.return_value = Mock(status_code=202)

        # Mock the responses for project retrieval and status
        mock_get.side_effect = [
            Mock(status_code=200, json=lambda: [{"id": 1, "name": "test_project", "path_with_namespace": "namespace/test_project"}]),
            Mock(status_code=200, json=lambda: {"export_status": "queued"}),
            Mock(status_code=200, json=lambda: {"export_status": "finished", "_links": {"api_url": "http://download_url"}}),
            Mock(status_code=200, iter_content=lambda chunk_size: [b'content_chunk1', b'content_chunk2', b'content_chunk3'])  # Simulate chunked content
        ]
